...
export default async function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 md:p-12 relative">
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-primary/10 rounded-full mix-blend-multiply filter blur-[100px] opacity-50 pointer-events-none"></div>
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        <header className="flex flex-col sm:flex-row sm:items-center justify-between mb-12 gap-4">
          ...
          <div className="flex items-center gap-4 bg-white/50 backdrop-blur-md border border-slate-200 rounded-full pl-4 pr-1.5 py-1.5 shadow-sm">
        ...
        <main>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {!companies || companies.length === 0 ? (
              <div className="col-span-full border-2 border-dashed border-slate-200 rounded-3xl p-12 flex flex-col items-center justify-center text-center bg-white/50">
              ...
              companies.map((company) => (
                <Link 
                  ...
                  key={company.id}
                  className="group bg-white border border-slate-200 hover:border-primary/30 rounded-3xl p-6 transition-all hover:shadow-xl hover:shadow-primary/5 hover:-translate-y-1 block relative overflow-hidden"
                >
                  ...
                  {company.tax_id && (
                    <span className="inline-flex items-center rounded-md bg-slate-100 px-2.5 py-0.5 text-xs font-semibold text-graytext border border-slate-200">
                      Tax ID: {company.tax_id}
                    </span>
                  )}
                  <div className="mt-6 flex items-center text-primary text-sm font-semibold group-hover:text-primary/80 transition-colors">
                    Manage Invoices 
