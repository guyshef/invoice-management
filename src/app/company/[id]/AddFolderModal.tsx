...
import { X, Folder, FolderPlus, Calendar } from 'lucide-react'
...
export function AddFolderModal({ companyId, currentYear, parentId, parentName, compact }: AddFolderModalProps) {
  ...
  const trigger = compact ? (
    <motion.button
      ...
      whileTap={{ scale: 0.95 }}
      onClick={openModal}
      title={isSubfolder ? `New subfolder in ${parentName}` : 'New Folder'}
      className="p-1.5 rounded-md text-slate-400 hover:bg-slate-100 hover:text-primary transition-colors"
    >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            >
              ...
              <div className="flex items-center justify-between mb-8 pt-2">
                <div>
                  <h3 className="text-2xl font-bold tracking-tight text-navy flex items-center gap-2.5">
                    {isSubfolder
                      ? <><FolderPlus className="w-6 h-6 text-primary" /> New Subfolder</>
                      : <><Folder className="w-6 h-6 text-primary" /> Add Folder</>
                    }
                  </h3>
                  <p className="text-graytext text-sm font-medium mt-1">
                    {isSubfolder
                      ? `Inside "${parentName}"`
                      : 'Organize your invoices by category'
                    }
                  </p>
                </div>
                <button
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    <input
                      ...
                      required
                      autoFocus
                      placeholder={isSubfolder ? 'e.g. Q1, Receipts' : 'e.g. Tax, Insurance'}
                ...
                >
                  {isPending ? 'Creating...' : isSubfolder ? 'Create Subfolder' : 'Create Folder'}
                </button>
              </form>
            </motion.div>
          </div>
